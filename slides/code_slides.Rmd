...
splt <- split(data, data$cluster)
 row1 <- as.numeric(unlist(lapply(splt, function(x) row.names(x))))
 row2 <- as.numeric(unlist(lapply(splt, function(x) x$cluster)))

which(diag(mm) %in% duplicated(diag(mm)), arr.ind=TRUE)
