...
data$cov <- unlist(lapply(splt, function(x) 
                  if(dim(x)[1] > 1) {
                    covariance_funtions(x, m = "mean_core",sd = "sd_core", n = "n_core", cov_type = "ROM") } else { NA}))

# Step 3: build matrix
m <- diag(data$v_lnRR)
m[which(duplicated(data[,cluster])==TRUE)+1, which(duplicated(data[,cluster])==TRUE)] <- data$cov[which(duplicated(data[,cluster])==TRUE)]
...
make_VCV_matrix(contrast_data, V = "v_lnRR", obs = "obs", cluster = "cluster", type = "cor")
