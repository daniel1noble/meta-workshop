...
**Does the correlation effect size for Exploration and Dispersal differ from Activity?** 

##### Answer {.tabset .tabset-fade .tabset-pills} 

We can see that Z-transformed correlations (*Zr*) between physiology and activity **do not differ significantly** from correlations between *Activity and Dispersal* and *Activity and Exploration*. The model also explains only `r orchaRd::r2_ml(overall_model)[1]`% ($R_{marginal}^2$) of the variation, so we can be confident that there is little predictive power from this moderator. 
